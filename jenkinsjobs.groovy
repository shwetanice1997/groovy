job("j-job1"){
        ...
        scm {
                 github('shwetanice1997/dynamic_cluster' , 'master')
             }
        triggers {
                scm("* * * * *")
        ...
        steps {
        shell('''sudo cp * /html/
sudo docker build -t shweta1997/http:latest .
sudo docker push shweta1997/http''')
      }
}
...
sudo kubectl create deployment myweb --image=shweta1997/http
sudo kubectl autoscale deployment myweb --min=10 --max=15 --cpu-percent=80
fi
if sudo kubectl get deployment -o wide | grep latest
then 
sudo kubectl set image deployment myweb http=shweta1997/http
else
sudo kubectl set image deployment myweb http=shweta1997/http:latest
fi
    ...
    extendedEmail {
      ...
      triggers {
        success{
          ...
          recipientList('shwetanice1997@gmail.com')
        }
        failure{
          attachBuildLog(true)
